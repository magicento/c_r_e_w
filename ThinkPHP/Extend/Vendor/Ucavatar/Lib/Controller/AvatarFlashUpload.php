...
class Controller_AvatarFlashUpload extends Controller_Base{
    ...
    public function showuploadAction() {
        ...
        if( $returnhtml == 1 ) {
            ...
            $avatarurlpathm = $this->config->uc_api. '/'. $this->config->avatardir. '/'. $this->get_avatar_filepath($uid, 'middle', '');
            $avatarurlpaths = $this->config->uc_api. '/'. $this->config->avatardir. '/'. $this->get_avatar_filepath($uid, 'small', '');
            $result .= '<script type="text/javascript">
                            function updateavatar() {
                                //修改两张小缩略图
                                var newpicm = "'.$avatarurlpathm.'";
                                var newpics = "'.$avatarurlpaths.'";
                                var oldpicm = parent.document.getElementById("useravatarm");
                                var oldpics = parent.document.getElementById("useravatars");
                                oldpicm.src = newpicm;
                                oldpics.src = newpics;
                                //修改左上角的个人图片
                                var oldpic = parent.document.getElementById("leftuseravar");
                                oldpic.src = newpicm;
