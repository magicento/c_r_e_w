...
<?php else: ?>
	...
	<div class="login-panel logoutpanel">
		...
		<div class="main-column usercenter myapp">
			<div class="rightcontentbox">
				<div class="headertitle"><div class="titleinfo">我的个人资料</div></div>
				<div class="secondtitle">
					<ul>
						<li class="active"><a href="<?php echo U('myapp');?>">我的应用</a></li>
				...
				<div class="realcontentbox">
					<?php if(empty($appslist)): ?><div class="noapplistforyou">
							你现在还没有应用，赶紧<a href="<?php echo U('User/allapp');?>" class="red">点击这里</a>添加应用开始学习吧！
						</div><?php endif; ?>
					<?php if(is_array($appslist)): $i = 0; $__LIST__ = $appslist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="itemapp <?php if($i%2 == 0): ?>even<?php endif; ?>">
							...
							<span class="apptitle"><?php echo ($vo["title"]); ?></span>
							<!-- <span title="<?php echo ($vo["id"]); ?>" class="gototest">模拟考试</span> -->
							<div class="clearfix"></div>
							<div class="clearfix"></div>
							<div class="appintro">
								<?php if(strlen($vo['intro']) > 38){ echo mb_substr($vo['intro'],0,38,'utf-8').'<a href="javascript:clicktodownload('.$vo[id].')">...详情</a>'; }else{ echo $vo['intro']; } ?>
							</div>
							<p>已经有<?php echo ($vo["hitnum"]); ?>人下载</p>
							<?php if(empty($vo["downloadurl"])): ?><button title="<?php echo ($vo["id"]); ?>" class="clicktobuy sutudyappcenter">点击进入</button>
							<?php else: ?>
								<button class="clicktodownload clicktobuy" onclick="clicktodownload(<?php echo ($vo["id"]); ?>)">点击下载</button><?php endif; ?>							
							</div>
